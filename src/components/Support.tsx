...
// Type definitions
type IssueType = "Technical Issue" | "Account Related Issue" | "Refund Request" | "Feedback" | "Other";
...
const supportSchema = z.object({
  name: z.string().min(2, "Name must be at least 2 characters"),
  email: z.string().email("Invalid email address"),
  issueType: z.enum(["Technical Issue", "Account Related Issue", "Refund Request", "Feedback", "Other"] as const),
  message: z.string().min(10, "Message must be at least 10 characters"),
});
...
const useCreateSupportRequest = () => {
  ...
  return useMutation<SupportRequest, Error, SupportFormData>({
    mutationFn: async (supportData) => {
      const { data } = await axiosInstance.post(
        "/support/create-support",
        supportData
...
// Form component for creating a new support request
const SupportRequestForm: React.FC<SupportRequestFormProps> = ({ onSubmit, onOpenChange }) => {
            ...
            <FormItem>
              ...
              <Select onValueChange={field.onChange} defaultValue={field.value}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="Account Related Issue">Account Related Issue</SelectItem>
                  <SelectItem value="Refund Request">Refund Request</SelectItem>
                  <SelectItem value="Feedback">Feedback</SelectItem>
                  <SelectItem value="Other">Other</SelectItem>
                </SelectContent>
...
// Table component to display support requests
const SupportRequestsTable: React.FC<SupportRequestsTableProps> = ({ requests, onDelete }) => (
  <Table>
    <TableHeader>
      <TableRow>
        <TableHead>Name</TableHead>
...
// Main Support Requests component
const SupportRequests: React.FC = () => {
  ...
  const totalPages = supportRequests?.totalPages || 1;
  const handleNextPage = () =>
    setPage((prev) => Math.min(prev + 1, totalPages));
  const handlePrevPage = () => setPage((prev) => Math.max(prev - 1, 1));
